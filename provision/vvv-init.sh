...
DB_NAME=${DB_NAME//[\\\/\.\<\>\:\"\'\|\?\!\*-]/}
LIVE_URL_VAR=`get_config_value 'live_url' "https://${DOMAIN}"`
SECURE_URL_VAR="https://${DOMAIN}"
...
if [ $LIVE_URL_VAR != $SECURE_URL_VAR ]; then
    echo "liveurl exist"
    cp -f "${VVV_PATH_TO_SITE}/provision/production_fallback-nginx.conf.tmpl" "${VVV_PATH_TO_SITE}/provision/vvv-nginx.conf"
    sed -i "s#{{LIVE_URL}}#${LIVE_URL_VAR}#" "${VVV_PATH_TO_SITE}/provision/vvv-nginx.conf"
    else
    echo "no liveurl is set"
fi
